...
## 2026-07-19 — Skip TEMPO output (human decision)
- Did: human decided to skip TEMPO emission entirely. The beatData/TEMPO backlog task is marked skipped. The converter will produce tracks, cues, loops, and playlists but no beat-grid markers.
- Verified: decision recorded in DECISIONS.md; BACKLOG updated.
- Next: generate the two playlist nodes and occurrence-based collection references; pass the complete exact semantic comparison.
- Blocked/Notes: `Inizio` cannot be derived exactly from beatData markers; exact-match rule prohibits approximate values. Revisit if the formula is reverse-engineered or acceptance criteria are relaxed.
